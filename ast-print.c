...
char* func_def_name(void *ptr){
    return ((ASTnode*)ptr)->name->name;
}

int func_def_nargs(void *ptr){
    return ((ASTnode*)ptr)->name->params;
}

char *func_def_argname(void *ptr, int n){
    scope* function = ((ASTnode*)ptr)->name->ptr;
    int arg_num = 1;
    sym* curr = function->symbols;
    while (arg_num != n) {
        curr = curr->next;
        arg_num++;
    }
...
void * stmt_if_expr(void *ptr){
    //expression will be kept in left
    return ((ASTnode*)ptr)->left;
}
